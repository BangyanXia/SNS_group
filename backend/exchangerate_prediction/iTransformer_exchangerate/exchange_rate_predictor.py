...
# 确保当前目录在sys.path中，这样可以正确导入experiments模块
...
class ExchangeRatePredictor:
    ...
    def fetch_exchange_rate_data(self):
        ...
        if not os.path.exists(file_path):
            ...
            for col in ["Open", "High", "Low", "Close"]:
                exchange_rate_data[col] = pd.to_numeric(exchange_rate_data[col])
    ...
    def get_model_args(self, data_path):
        ...
        args.data_path = os.path.basename(data_path)  # 只使用文件名，不使用完整路径
        args.features = 'M'
        args.target = 'Close'
        args.freq = 'd'
        ...
        args.target_data_path = os.path.basename(data_path)
    ...
    def predict(self):
        ...
        try:
            ...
            try:
                ...
                print("尝试导入Exp_Long_Term_Forecast失败，正在调整导入路径...")
                # 如果直接导入失败，尝试调整sys.path
                if current_dir not in sys.path:
                    sys.path.append(current_dir)
                # 再次尝试导入
                from experiments.exp_long_term_forecasting import Exp_Long_Term_Forecast
    ...
    def _simple_predict(self):
        ...
        if not os.path.exists(file_path):
            # 如果数据文件不存在，先获取数据
            file_path = self.fetch_exchange_rate_data()
        ...
        close_pct_change = recent_data['Close'].pct_change().mean()
        ...
        # 计算典型的日内波动率
        avg_daily_range = (recent_data['High'] - recent_data['Low']).mean()

        # 预测结果
        result = []
        today = datetime.now().date()

        for i in range(min(self.prediction_days, getattr(self, 'user_prediction_days', self.prediction_days))):
            # 每天的预测值基于简单的趋势延续
            prediction_date = today + pd.Timedelta(days=i + 1)
            predicted_close = last_close * (1 + close_pct_change * (i + 1))

            # 假设开盘接近前一天收盘
            predicted_open = predicted_close / (1 + close_pct_change)

            # 高低点基于平均日内波动
            predicted_high = predicted_close + (avg_daily_range / 2)
            predicted_low = predicted_close - (avg_daily_range / 2)
...
if __name__ == "__main__":
    ...
    with open(output_file, 'r') as f:
        ...
        for day in results[:3]:  # Show first 3 days
            print(f"Date: {day['date']}, Close: {day['close']:.4f}, Note: {day.get('note', '')}")
        if len(results) > 3:
            print("...")